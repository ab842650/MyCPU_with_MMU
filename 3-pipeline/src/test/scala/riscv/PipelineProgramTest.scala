...
class PipelineProgramTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  for (cfg <- PipelineConfigs.All) {
    ...
    it should "compute rsqrt_fast and match reference" in {
      runProgram("rsqrt_fast.asmbin", cfg) { c =>

        c.clock.setTimeout(200000)

        c.clock.step(50000)

        def check(addr: Int, expected: Int): Unit = {
          c.io.mem_debug_read_address.poke(addr.U)
          c.clock.step()
          c.io.mem_debug_read_data.expect(expected.U)
        }
        check(0x4,  0x00018000) // rsqrt(1)
        check(0x8,  0x0000B505) // rsqrt(2)
        check(0xC,  0x00008000) // rsqrt(4)
        check(0x10, 0x00004000) // rsqrt(16)
        check(0x14, 0x00001000) // rsqrt(256)
      }
    }
